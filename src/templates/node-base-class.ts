...
export class NodeBaseClass extends BaseFunctions {
  ...
  protected setupNodes(
    id: string,
    inputsClasslist: Array<string>,
    outputsClasslist: Array<string>
  ): LocalNodesModel {
  ...
  public LoadNodesFromConfig(
     ...
     */
    const setups: LocalNodesModel = this.setupNodes(
      dataNode.ID,
      ["inputs"],
      ["outputs"]
    );
     ...
     */
    if (dataNode.Inputs) {
      Object.keys(dataNode.Inputs).map((input_item: string, index: number) => {
        /**
         * Input item
         */
