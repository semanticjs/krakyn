...
export class DataFlowBaseClass extends BaseFunctions {
  ...
  public DragEnd(event: DragEvent): void {
    ...
    if (VariablesUtils.Dragging) {
      if (
        VariablesUtils.PosXStart != e_pos_x ||
        VariablesUtils.PosYStart != e_pos_y
      ) {
    ...
    if (VariablesUtils.DragPoint) {
      ...
      if (
        ...
        this.Dispatch(
          "rerouteMoved",
          VariablesUtils.SelectedElement.parentElement.classList[2].slice(14)
        );
      }
    }

    if (VariablesUtils.EditorIsSelected) {
      VariablesUtils.CanvasX =
        VariablesUtils.CanvasX + -(VariablesUtils.PosX - e_pos_x);
      VariablesUtils.CanvasY =
        VariablesUtils.CanvasY + -(VariablesUtils.PosY - e_pos_y);
    ...
    if (VariablesUtils.Connection === true) {
      ...
      if (
        ...
        var output_id =
          VariablesUtils.SelectedElement.parentElement.parentElement.id;
        var output_class = VariablesUtils.SelectedElement.classList[1];

        if (output_id !== input_id && input_class !== false) {
          if (
            VariablesUtils.MainContainer.querySelectorAll(
              ".connection.node_in_" +
                input_id +
                ".node_out_" +
                output_id +
             ...
             */
            let outputElement: Array<any> = this.activeModule(
             ...
             */
            let inputElement: Array<any> = this.activeModule(
              VariablesUtils.ActiveModule
            ).Data.filter((obj: NodeModel) => {
            ...
            if (this.canConnect(event, inputElement, outputElement)) {
              VariablesUtils.ConnectionElement.classList.add(
                "node_in_" + input_id
              );
              VariablesUtils.ConnectionElement.classList.add(
                "node_out_" + output_id
              ...
              outputElement[0].Outputs[output_class].Connections.push({
                node: id_input,
                output: input_class,
              });
              inputElement[0].Inputs[input_class].Connections.push({
                node: id_output,
                input: output_class,
              });
              ...
              const closestSVG =
                VariablesUtils.ConnectionElement.closest("svg");
    ...
    if (VariablesUtils.Dragging) {
      var x =
        ((VariablesUtils.PosX - e_pos_x) *
          VariablesUtils.PreCanvas.clientWidth) /
        (VariablesUtils.PreCanvas.clientWidth * VariablesUtils.Zoom);
      var y =
        ((VariablesUtils.PosY - e_pos_y) *
          VariablesUtils.PreCanvas.clientHeight) /
        (VariablesUtils.PreCanvas.clientHeight * VariablesUtils.Zoom);
      VariablesUtils.PosX = e_pos_x;
      VariablesUtils.PosY = e_pos_y;

      VariablesUtils.SelectedElement.style.top =
        VariablesUtils.SelectedElement.offsetTop - y + "px";
      VariablesUtils.SelectedElement.style.left =
        VariablesUtils.SelectedElement.offsetLeft - x + "px";

      /**
       * Get the selected item
       */
      let selectedElement = this.activeModule(
        VariablesUtils.ActiveModule
      ).Data.filter((obj: NodeModel) => {
    ...
    if (VariablesUtils.DragPoint) {
      ...
      var pos_x =
        VariablesUtils.PosX *
          (VariablesUtils.PreCanvas.clientWidth /
            (VariablesUtils.PreCanvas.clientWidth * VariablesUtils.Zoom)) -
        VariablesUtils.PreCanvas.getBoundingClientRect().x *
          (VariablesUtils.PreCanvas.clientWidth /
            (VariablesUtils.PreCanvas.clientWidth * VariablesUtils.Zoom));
      var pos_y =
        VariablesUtils.PosY *
          (VariablesUtils.PreCanvas.clientHeight /
            (VariablesUtils.PreCanvas.clientHeight * VariablesUtils.Zoom)) -
        VariablesUtils.PreCanvas.getBoundingClientRect().y *
          (VariablesUtils.PreCanvas.clientHeight /
            (VariablesUtils.PreCanvas.clientHeight * VariablesUtils.Zoom));
      ...
      const nodeUpdateIn =
        VariablesUtils.SelectedElement.parentElement.classList[1].slice(13);
      const output_class =
        VariablesUtils.SelectedElement.parentElement.classList[3];
      const input_class =
        VariablesUtils.SelectedElement.parentElement.classList[4];

      let numberPointPosition =
        Array.from(
          VariablesUtils.SelectedElement.parentElement.children
      ...
      const searchConnection = this.activeModule(
        VariablesUtils.ActiveModule
      ).Data[nodeId].outputs[output_class].Connections.findIndex(function (
        item: any,
        i: any
      ) {
      ...
      this.activeModule(VariablesUtils.ActiveModule).Data[nodeId].outputs[
        output_class
  ...
  public Click(event: any): any {
    ...
    if (VariablesUtils.EditorMode === "fixed") {
      ...
      if (target.closest(".drawflow_content_node") != null) {
        VariablesUtils.SelectedElement = target.closest(
    ...
    switch (VariablesUtils.SelectedElement.classList[0]) {
      case VariablesUtils.NodeClass:
        ...
        if (VariablesUtils.SelectedNode != VariablesUtils.SelectedElement) {
          this.Dispatch(
            "nodeSelected",
            VariablesUtils.SelectedElement.id.slice(5)
          );
        }
      ...
      case "main-path":
        ...
        const listclassConnection =
          VariablesUtils.SelectedConnection.parentElement.classList;
        ...
        if (VariablesUtils.RerouteFixCurvature) {
          VariablesUtils.SelectedConnection.parentElement
  ...
  public Contextmenu(event: any): any {
    ...
    event.preventDefault();
    if (
      VariablesUtils.EditorMode === "fixed" ||
      VariablesUtils.EditorMode === "view"
    ) {
      return false;
    }
    if (
      VariablesUtils.PreCanvas.getElementsByClassName("drawflow-delete").length
    ) {
      VariablesUtils.PreCanvas.getElementsByClassName(
        "drawflow-delete"
      )[0].remove();
    }
  ...
  public KeyDown(event: any): any {
    ...
    if (
      ...
      if (VariablesUtils.SelectedNode != null) {
        if (
          VariablesUtils.FirstClickedElement.tagName !== "INPUT" &&
          VariablesUtils.FirstClickedElement.tagName !== "TEXTAREA" &&
          VariablesUtils.FirstClickedElement.hasAttribute("contenteditable") !==
            true
        ) {
          this.removeNodeId(VariablesUtils.SelectedNode.id);
        }
  ...
  public UpdateNodeValue(event: any): void {
    ...
    for (var i = 0; i < attr.length; i++) {
      if (attr[i].nodeName.startsWith("df-")) {
        ...
        var target = this.activeModule(VariablesUtils.ActiveModule).Data[
          event.target
