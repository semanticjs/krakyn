...
export class DrawingUtils {
  ...
  public static DrawConnection(
    ele: any,
    callback: (val: string, output: {}) => void
  ): void {
    console.log("DrawConnection");
    /**
     * Create connection SVG element
     */
    const connection: SVGSVGElement = document.createElementNS(
  ...
  public static UpdateConnection(eX: number, eY: number): void {
    ...
    var line_x =
      variables.SelectedElement.offsetWidth / 2 +
      (variables.SelectedElement.getBoundingClientRect().x -
        precanvas.getBoundingClientRect().x) *
        precanvasWitdhZoom;
    var line_y =
      variables.SelectedElement.offsetHeight / 2 +
      (variables.SelectedElement.getBoundingClientRect().y -
        precanvas.getBoundingClientRect().y) *
        precanvasHeightZoom;

    var x =
      eX *
        (variables.PreCanvas.clientWidth /
          (variables.PreCanvas.clientWidth * variables.Zoom)) -
      variables.PreCanvas.getBoundingClientRect().x *
        (variables.PreCanvas.clientWidth /
          (variables.PreCanvas.clientWidth * variables.Zoom));
    var y =
      eY *
        (variables.PreCanvas.clientHeight /
          (variables.PreCanvas.clientHeight * variables.Zoom)) -
      variables.PreCanvas.getBoundingClientRect().y *
        (variables.PreCanvas.clientHeight /
          (variables.PreCanvas.clientHeight * variables.Zoom));

    var curvature = variables.Curvature;
    var lineCurve = DrawingUtils.CreateCurvature(
      line_x,
      line_y,
      x,
      y,
